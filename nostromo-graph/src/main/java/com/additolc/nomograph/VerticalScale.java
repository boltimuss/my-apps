...
@SuperBuilder
public class VerticalScale extends AbstractScale {

	public VerticalScale(AbstractScaleBuilder<?, ?> b) {
		super(b);
	}
	...
	public double getDataPointForSlideValue(double slideValue)
	{
		for (Section section:sections)
		{
			if (slideValue >= section.getStartLocation() && slideValue <= section.getEndLocation())
			{
				double deltaY = Math.abs(section.getStartLocation() - section.getEndLocation());
				double deltaValue = Math.abs(section.getStartValue() - section.getEndValue());
				
				double percentage = Math.abs(slideValue - section.getStartLocation()) / deltaY;
	...
	public void drawDraggableNotch(GraphicsContext gc)
	{
		...
		double offsetX = mmPerPixel * scaleOffset.getX();
		double offsetY = mmPerPixel * scaleOffset.getY();
		if (draggableX == -99) draggableX = offsetX + (mmPerPixel * draggableOffset.getX());
		if (draggableY == -99) draggableY = offsetY + (mmPerPixel*mmStartOffset) + (mmPerPixel * draggableOffset.getY());
		...
		if (showDraggable)
		{
			...
			double value = getDataPointForSlideValue(draggableY);
			if (value > -999)
			{
